...
{	
	...
	{
		//int pos = 2;
		if (ret[i] == '\\' && i + 1 < ret.length()) { //如果ret[i]的字符为'\'
			int pos = 2;
			if (ret[i + 1] == '\"')             // /" 
				result += '"';
			else if (ret[i + 1] == '\\')
				...
				result += '\t';
			else if (ret[i + 1] == 'u' && i + 5 < ret.length()){  //十六进制符号
				long v = 0;
				pos += 4;
				for (int j = 0; j < 4; j++) {
					if (ret[i + 2 + j] >= '0' && ret[i + 2 + j] <= '9') v += ret[i + 2 + j];
					if (ret[i + 2 + j] >= 'a' && ret[i + 2 + j] <= 'f') v += ret[i + 2 + j] - 'a' + 10;
				}
				result += (char)v;
			}
...
//输出空格
//在解析过程中，如对象的“名称/值”在左括号的下方，并且有一定的缩进，使得看起来清晰明了
string value::space(int n)
{
	string sp = " ";
	while (n--) sp += " ";
	return sp;
...
//将字符串进行解析，其中遇到\n \t等这些符号是，能够顺利的直接输出出来
string value::tostring(int n)
{
	if (type == MySTRING) return s;
	if (type == MyNUMBER) return s;
	if (type == MYBOOL) return s;
	if (type == MyNULL) return "null";
	//对象（object） 是一个无序的“‘名称/值’对”集合。一个对象以“{”（左括号）开始，“}”（右括号）结束。
	//每个“名称”后跟一个“:”（冒号）；“‘名称/值’ 对”之间使用“,”（逗号）分隔。
	if (type == MyOBJECT) {		//当类型为MyOBJECT时它的输出格式
		string object_s = ("{\n");
		for (auto i = 0; i < object_map.size(); i++) {
			object_s += space(n) + string("\"") + object_map[i].first + string("\": ") + object_map[i].second.tostring(n + 1) + string(i == object_map.size() - 1 ? "" : ",") + string("\n");
		}
	object_s += space(n-1) + string("}");
	return object_s;
	}
...
void value::set_string(string value_s)
{
	s = value_s;
}
...
{
	if (m.find(s) == m.end()) return value();
	//if (m.find(s) == m.end())	return value();
	// m[s];
	return object_map[m[s]].second;
}

MyType value::get_type()
{
	return type;
}
 
void value::set_type(MyType tp)
{
	type = tp;
}

void value::add_enement(value v)
{
	vec.push_back(v);
}

void value::add_property(string key, value v)
{
	m[key] = object_map.size();
	object_map.push_back(make_pair(key,v));
}
...
double value::get_exponent_value()
{
	/*double k;
	int i = 0;
	stringstream ss;
...
struct parse::push
{
	string json_value;
	parse_type type;
	push(string json_value = "", parse_type type = UNKNOWN) :json_value(json_value), type(type){}
};
...
//将得到的数据压入容器里 
vector<parse::push> parse::push_data(string source)
{
	//int k = 0;
	vector<push> pushs;
	...
	while (index >= 0){
		...
		while (k < str.length()) {
			if (str[k] == '"') {
				int k_ = k + 1;
				while (k_ < str.length() && str[k_] != '"' || str[k_ - 1] == '\\') k_++;
				pushs.push_back(push(str.substr(k + 1, k_ - k - 1), STRING));
				k = k_ + 1;
				continue;
			}
			if (str[k] == ',') {
				pushs.push_back(push(",", COMMA));
				k++;
				continue;
			}
			if (str[k] == 't' && k + 3 < str.length() && str.substr(k, 4) == "true") {
				pushs.push_back(push("true", BOOLEAN));
				k += 4;
				continue;
			}
			if (str[k] == 'f' && k + 4 < str.length() && str.substr(k, 5) == "false") {
				pushs.push_back(push("false", BOOLEAN));
				k += 5;
				continue;
			}
			if (str[k] == 'n' && k + 3 < str.length() && str.substr(k, 4) == "null") {
				pushs.push_back(push("null", NUL));
				k += 4;
				continue;
			}
			if (str[k] == '{') {
				pushs.push_back(push("{", LEFR_BIG_BRACKET));
				k++;
				continue;
			}
			if (str[k] == '}') {
				pushs.push_back(push("}", RIGHT_BIG_BRACKET));
				k++;
				continue;
			}
			if (str[k] == '['){
				pushs.push_back(push("[", LEFR_MID_BRACKET));
				k++;
				continue;
			}
			if (str[k] == ']') {
				pushs.push_back(push("]", RIGHT_MID_BRACKET));
				k++;
				continue;
			}
			if (str[k] == ':') {
				pushs.push_back(push(":", COLON));
				k++;
				continue;
			}
			if (str[k] == '-' || (str[k] >= '0' && str[k] <= '9')) {
				int k_ = k;
				if (str[k_] == '-') k_++;
				while (k_ <= str.length() && (str[k_] >= '0' && str[k_] <= '9') ||( str[k_] == '.' || str[k_] == 'e' || str[k_] == 'E' || str[k_] == '+' || str[k_] == '-' )) k_++;
				pushs.push_back(push(str.substr(k, k_ - k), NUMBER));
				k = k_;
				continue;
			}
			pushs.push_back(push(str.substr(k), UNKNOWN));
			k = str.length();
...
//设置所属类型
value parse::json_parse(vector<push> v, int i, int &r)
{
	value result;
	if (v[i].type == LEFR_BIG_BRACKET) {
		result.set_type(MyOBJECT);
		int k = i + 1;
		while (v[k].type != RIGHT_BIG_BRACKET) {
			string key = v[k].json_value;
			k += 2;
			int j = k;
			value vv = json_parse(v, k, j);
			result.add_property(key, vv);
			k = j;
			if (v[k].type == COMMA) k++;
		}
		r = k + 1;
	...
	if (v[i].type == LEFR_MID_BRACKET) {
		result.set_type(MyARRAY);
		int k = i + 1;
		while (v[k].type != RIGHT_MID_BRACKET) {
			int j = k;
			value vv = json_parse(v, k, j);
			result.add_enement(vv);
			k = j;
			if (v[k].type == COMMA) k++;
		}
		r = k + 1;
	...
	if (v[i].type == BOOLEAN) {
		result.set_type(MYBOOL);
		result.set_string(v[i].json_value);
		r = i + 1;
		return result;
	}
	if (v[i].type == STRING) {
		result.set_type(MySTRING);
		result.set_string(v[i].json_value);
		r = i + 1;
		return result;
	}
	if (v[i].type == NUL) {
		result.set_type(MyNULL);
		result.set_string("null");
		r = i + 1;
		return result;
	}
	if (v[i].type == NUMBER) {
		result.set_type(MyNUMBER);
		result.set_string(v[i].json_value);
		r = i + 1;
		return result;
	}
...
value parse::parser(const string &str)
{
	int k;
	return json_parse(push_data(str), 0, k);
}
